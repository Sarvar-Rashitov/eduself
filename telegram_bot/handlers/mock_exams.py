...
async def mock_exams_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "*Mock Imtihonlar*\n\nDTM uslubidagi sinov imtihonlari.\nImtihonni tanlang:"
...
async def mock_exam_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("Orqaga", callback_data="mock_exams")]
    ]
...
async def start_mock_exam(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not questions:
        await query.answer("Bu imtihonda savollar yoq!", show_alert=True)
        return
    ...
    await show_mock_question(query, context, questions[0], 1, len(questions))


async def show_mock_question(query, context, question, num, total):
...
async def handle_mock_inline_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for i, ans in enumerate(answers):
            letter = chr(65 + i)
            if ans.is_correct:
                text += f"[Togri] *{letter})* {ans.text}\n"
            elif ans.id == answer_id and not ans.is_correct:
                text += f"[Xato] *{letter})* {ans.text}\n"
            else:
        ...
        if answer.is_correct:
            ...
            for letter, ans_id in current_answers.items():
                if correct_answer and ans_id == correct_answer.id:
                    ...
                    break
            text += f"\n\n*Notogri!* Togri javob: *{correct_letter}*"
        
        keyboard = [[InlineKeyboardButton("Keyingi savol", callback_data="mock_next")]]
...
async def handle_mock_skip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer("Savol otkazib yuborildi")
...
async def finish_mock_from_callback(query, context):
    session = context.user_data.get('test_session')
    if not session:
    ...
    status = "OTDINGIZ!" if passed else "OTMADINGIZ"
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("Imtihonlarga qaytish", callback_data="mock_exams")]
    ]
...
async def mock_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("Orqaga", callback_data=f"mock_{exam_id}")]]
